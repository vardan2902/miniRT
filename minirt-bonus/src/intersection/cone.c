...
/*   Updated: 2025/06/14 20:48:47 by ysaroyan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static float	get_t_side(t_ray *ray, t_cone *cone)
{
	t_roots			roots;
	t_coefficients	coeff;
	float			t_side;
	...
	t_side = fminf(roots.t1, roots.t2);
	if (t_side < EPSILON)
		t_side = fmaxf(roots.t1, roots.t2);
	return (t_side);
}
...
bool	intersect_cone(t_ray *ray, t_object *object, t_hit *hit)
{
	...
	normal = v_normalize(*cone->orientation);
	props.t_side = get_t_side(ray, cone);
